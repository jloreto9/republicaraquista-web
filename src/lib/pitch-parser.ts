...
export function calculateSimpleLI(inning: number, outs: number, scoreDiff: number): number {
  let baseLi = 1.0;
  if (inning >= 7) {
...
export function sumInnings(ipStrings: (string | number)[]): { ipString: string; totalInningsFloat: number } {
  let totalOuts = 0;
  for (const s of ipStrings) {
    const str = String(s || "0.0").trim();
    const parts = str.split(".");
    const full = parseInt(parts[0], 10) || 0;
    const partial = parts.length > 1 ? parseInt(parts[1], 10) || 0 : 0;
    totalOuts += full * 3 + partial;
  }
  const fullInnings = Math.floor(totalOuts / 3);
  const remOuts = totalOuts % 3;
  return {
    ipString: `${fullInnings}.${remOuts}`,
    totalInningsFloat: totalOuts / 3,
  };
}

export function parseLiveFeedPitches(liveData: any, pitcherId: number): PitchDetail[] {
...
export function computePitchMetrics(parsedPitches: PitchDetail[]) {
  const totalPitches = parsedPitches.length;
  const hasStatcast = parsedPitches.some((p) => p.speed !== null && p.ivb !== null);
  ...
  const cswPct = totalPitches > 0 ? ((calledCount + whiffsCount) / totalPitches) * 100 : 0;
  ...
  return {
    totalPitches,
    hasStatcast,
    cswPctStr: `${cswPct.toFixed(1)}%`,
    whiffPctStr: `${whiffPct.toFixed(1)}%`,
    strikesCount,
    statcastTable,
    pbpTable,
    pbpKpis,
